...
extended_consumption_frac_t_series = horzcat(ones(1,num_negative_indicies),consumption_ratio_t_series);


extended_time_desc = birth_years(1):5:time_desc_2(end);

personal_sampling = NaN(length(birth_years),length(life_time_lengths_years));
personal_sampling_fraction = NaN(length(birth_years),length(life_time_lengths_years));
 ...
 for lifetime_window_lengths_i = 1:length(life_time_lengths_years)
     for birth_year_i = 1:length(birth_years)
         ...
         personal_sampling(birth_year_i,lifetime_window_lengths_i) = mean(extended_consumption_diff_t_series(first_year_sample_i:last_year_sample_i));
         personal_sampling_fraction(birth_year_i,lifetime_window_lengths_i) = mean(extended_consumption_frac_t_series(first_year_sample_i:last_year_sample_i));
     end
 end
...
%linspace(min_color_range,max_color_range,num_contours)


    %contourf(birth_years,life_time_lengths_years,personal_sampling',contour_range_values, 'LineStyle','none')
    contourf(birth_years,life_time_lengths_years,personal_sampling_fraction',contour_range_values, 'LineStyle','none')
    
    %contour(birth_years,life_time_lengths_years,personal_sampling_fraction',[1.0001 1.00001],'k','LineStyle','-','LineWidth',4);
    contour(birth_years,life_time_lengths_years,personal_sampling_fraction',[1.000 1.000],'k','LineStyle','-','LineWidth',4);

    caxis(contour_range_values)

    xlabel('Birth Year')
    ylabel('Life Length (years)')
    title('Policy Effect on Lifetime Average Consumption')
...
contour_range_values_neg = linspace(-50,50,20);
contour_range_values_pos = linspace(-6000,6000,20);
...
colormap(rgb3)
    ...
    xlabel('Birth Year')
    ylabel('Life Length (years)')
    title('Mitigation impact on Lifetime Average Consumption')
    
    h = colorbar;
    ylabel(h,'lifetime mean consumption difference');
