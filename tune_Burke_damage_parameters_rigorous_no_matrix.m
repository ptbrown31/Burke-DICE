...
time_desc_original_data = 2005:5:2100;

SSP_all_data = NaN(length(RCPs),length(time_desc_original_data),length(var_names),length(SSPs));
for SSPs_i = 1:length(SSPs)
    for rcp_i = 1:length(RCPs)
...
for SSPs_i = 1:length(SSPs)
    for rcp_i = 1:length(RCPs)
        for time_i = 1:length(time_desc_original_data)
...
%% bump up time to match DICE2016

time_desc_DICE2016_good_inds = find(time_desc_original_data >= 2015);

time_desc_cutoff_to_DICE2016 = time_desc_original_data(time_desc_DICE2016_good_inds);

SSP_all_data = SSP_all_data(:,time_desc_DICE2016_good_inds,:,:,:);

time_desc_2050_i = find(time_desc_cutoff_to_DICE2016 == 2050);
time_desc_2100_i = find(time_desc_cutoff_to_DICE2016 == 2100);
...
% PRODUCTIVITY
    ...
    A = A(1:length(time_desc_cutoff_to_DICE2016));
    Ag = Ag(1:length(time_desc_cutoff_to_DICE2016));
    
%% set up values

        damage_dep_rate_coeff_on_T_1 = 0.108;
        damage_dep_rate_coeff_on_T_2 = 0.013;

%         damage_dep_rate_coeff_on_T_1 = 0;
%         damage_dep_rate_coeff_on_T_2 = 0;

    dam_int = 0.00;
        
        %damage_TFP_growth_coeff_on_T_1 = 0.003;
        damage_TFP_growth_coeff_on_T_1 = 0.0055;
...
deltaks_f_GMST = dam_int + damage_dep_rate_coeff_on_T_1.*GMSTs - damage_dep_rate_coeff_on_T_2.*GMSTs.^2;

% deltaks_f_GMST = damage_dep_rate_coeffs_on_T_central.*GMSTs;
 deltaks_f_GMST(deltaks_f_GMST < deltak) = deltak;

Ag_f_GMST = mean(Ag) - damage_TFP_growth_coeff_on_T_1.*GMSTs;
        ...
        hold on
            ...
            legend('DICE-Default','DICE-Burke','Location','NorthWest')
        ...
        hold on
            ...
            legend('DICE-Default (time-mean)','DICE-Burke','Location','SouthWest')
...
%% calculate 
        ...
        %define empties for this paramater combo
            ...
            A_with_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));
            K_with_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));
            Y_with_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));
            Y_per_L_with_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));

            Y_without_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));
            Y_per_L_without_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));
            
            Y_per_Ls_vs_no_damage = NaN(length(RCPs),length(time_desc_cutoff_to_DICE2016),length(SSPs));
            
            errors_between_my_calc_and_Burke_fit = NaN(length(RCPs),length(SSPs),2);
            ...
            for rcp_i = 1:length(RCPs)    
                ...
                if sum(isnan(Y)) == 0
                    ...
                    % K_next = tstep * saverate_now * Y_now + (1 - cap_dep_rate) ^ step * K_now
                        ...
                        save_rate = NaN(1,length(time_desc_cutoff_to_DICE2016));

                        for time_i = 1:length(time_desc_cutoff_to_DICE2016)-1
...
%                         plot(save_rate)
            ...
            %% calculate damages, accumulating over time
                    ...
                    Y_without_damage(rcp_i,1,SSPs_i) = Y(1)./squeeze(SSP_all_data_raw(rcp_i,1,2,SSPs_i));
                        ...
                        for time_i = 2:length(time_desc_cutoff_to_DICE2016)
                    ...
                    %                A_damaged = S1(23)/(1-Ag_damaged);  %S1(23) is the previous timesteps damaged TFP
                                     ...
                                     Ag_damaged_now = Ag(time_i-1) - damage_TFP_growth_coeff_on_T_1*global_temp_now;
                    ...
                    %                deltak_damaged_now = deltak + damage_dep_rate_coeff_on_T*global_temp_now.^3;
                                    ...
                                    deltak_damaged_now = dam_int + damage_dep_rate_coeff_on_T_1.*global_temp_now - damage_dep_rate_coeff_on_T_2.*global_temp_now.^2;
...
%                                     if deltak_damaged_now < deltak; deltak_damaged_now = deltak; end
                    ...
                    %               S2(7) = A_damaged * (S2(1) ^ gama) * (L(t1 + 1)/1000)^(1 - gama);
                            ...
                            %calculate the damaged gross output per capita
                                    ...
                                    Y_per_Ls_vs_no_damage(rcp_i,time_i,SSPs_i) = -100*(1-(Y_per_L_with_damage(rcp_i,time_i,SSPs_i)./Y_per_L_without_damage(rcp_i,time_i,SSPs_i)));
                        ...
                        y = y(isnan(y) == 0);
                            ...
                            if isempty(x) == 0 && isempty(y) == 0
                                ...
                                myfit = fit(x,y,myfittype);
                                        ...
                                        my_damage_at_this_Temp = Y_per_Ls_vs_no_damage_points(rcp_i,time_horizs_i,SSPs_i);
...
%                                             (my_damage_at_this_Temp - Burke_damage_at_this_Temp).^2;
                                        errors_between_my_calc_and_Burke_fit(rcp_i,SSPs_i,time_horizs_i) = ...
                                            (my_damage_at_this_Temp - Burke_damage_at_this_Temp).^2;
                            end
                    end
                end
                ...
                for time_horizs_i = 1:length(time_horzs)
                        ...
                        if isempty(x) == 0 && isempty(y) == 0
                         ...
                         scatter(Temp_points(:,time_horizs_i,SSPs_i),...
                                 Y_per_Ls_vs_no_damage_points(:,time_horizs_i,SSPs_i),...
                                 100,'k','s','filled')
                    ...
                    rmse = sqrt(nanmean(nanmean(squeeze(errors_between_my_calc_and_Burke_fit(:,SSPs_i,:)))));
